...
create policy "courses_select" on public.courses
  for select to authenticated using (is_published = true or teacher_id = auth.uid());
...
as $fn$
  select exists (
    select 1 from public.courses where id = _course_id and teacher_id = auth.uid()
  );
$fn$;
...
create policy "units_insert" on public.units
  for insert to authenticated with check (public.is_course_teacher(course_id));

drop policy if exists "units_update" on public.units;
create policy "units_update" on public.units
  for update to authenticated using (public.is_course_teacher(course_id)) with check (public.is_course_teacher(course_id));

drop policy if exists "units_delete" on public.units;
create policy "units_delete" on public.units
  for delete to authenticated using (public.is_course_teacher(course_id));
...
as $fn$
  select exists (
    ...
    where l.id = _lesson_id and c.teacher_id = auth.uid()
  );
$fn$;
...
create policy "lessons_select" on public.lessons
  for select to authenticated using (
    exists (
      ...
      join public.courses c on c.id = u.course_id
      where u.id = unit_id and (c.is_published = true or c.teacher_id = auth.uid())
    )
  );

drop policy if exists "lessons_insert" on public.lessons;
create policy "lessons_insert" on public.lessons
  for insert to authenticated with check (public.is_lesson_teacher(unit_id));

drop policy if exists "lessons_update" on public.lessons;
create policy "lessons_update" on public.lessons
  for update to authenticated using (public.is_lesson_teacher(id)) with check (public.is_lesson_teacher(id));

drop policy if exists "lessons_delete" on public.lessons;
create policy "lessons_delete" on public.lessons
  for delete to authenticated using (public.is_lesson_teacher(id));
...
create policy "videos_select" on public.videos
  for select to authenticated using (
    exists (
      ...
      join public.units u on u.id = l.unit_id
      join public.courses c on c.id = u.course_id
      where l.id = lesson_id and (c.is_published = true or c.teacher_id = auth.uid())
    )
  );

drop policy if exists "videos_insert" on public.videos;
create policy "videos_insert" on public.videos
  for insert to authenticated with check (public.is_lesson_teacher(lesson_id));

drop policy if exists "videos_update" on public.videos;
create policy "videos_update" on public.videos
  for update to authenticated using (public.is_lesson_teacher(lesson_id)) with check (public.is_lesson_teacher(lesson_id));

drop policy if exists "videos_delete" on public.videos;
create policy "videos_delete" on public.videos
  for delete to authenticated using (public.is_lesson_teacher(lesson_id));
...
create policy "lesson_files_insert" on public.lesson_files
  for insert to authenticated with check (public.is_lesson_teacher(lesson_id));

drop policy if exists "lesson_files_update" on public.lesson_files;
create policy "lesson_files_update" on public.lesson_files
  for update to authenticated using (public.is_lesson_teacher(lesson_id)) with check (public.is_lesson_teacher(lesson_id));

drop policy if exists "lesson_files_delete" on public.lesson_files;
create policy "lesson_files_delete" on public.lesson_files
  for delete to authenticated using (public.is_lesson_teacher(lesson_id));
...
as $fn$
  select exists (
    ...
    join public.units u on u.id = l.unit_id
    join public.courses c on c.id = u.course_id
    where e.id = _exam_id and c.teacher_id = auth.uid()
  );
$fn$;
...
create policy "exams_update" on public.exams
  for update to authenticated using (public.is_exam_teacher(id)) with check (public.is_exam_teacher(id));

drop policy if exists "exams_delete" on public.exams;
create policy "exams_delete" on public.exams
  for delete to authenticated using (public.is_exam_teacher(id));
...
create policy "exam_attempts_select" on public.exam_attempts
  for select to authenticated using (student_id = auth.uid() or public.is_exam_teacher(exam_id));
...
create policy "covers_public_read" on storage.objects
  for select to authenticated using (bucket_id = 'course-covers');

drop policy if exists "covers_teacher_upload" on storage.objects;
create policy "covers_teacher_upload" on storage.objects
  for insert to authenticated with check (bucket_id = 'course-covers' and public.has_role(auth.uid(), 'teacher'));

drop policy if exists "covers_teacher_delete" on storage.objects;
create policy "covers_teacher_delete" on storage.objects
  for delete to authenticated using (bucket_id = 'course-covers' and public.has_role(auth.uid(), 'teacher'));
...
create policy "lesson_files_teacher_upload" on storage.objects
  for insert to authenticated with check (bucket_id = 'lesson-files' and public.has_role(auth.uid(), 'teacher'));
...
create policy "lesson_files_teacher_delete" on storage.objects
  for delete to authenticated using (bucket_id = 'lesson-files' and public.has_role(auth.uid(), 'teacher'));
